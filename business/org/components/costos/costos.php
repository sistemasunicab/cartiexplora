...
while ($row_seccion = $res_seccion->fetch_assoc()) {
    ...
    $html .= '<section class="bg-bold-blue p-4 mb-4">';
    ...
    $res_imagenes = $mysqli1->query($sentencia . "130");
    while ($row_sentencia = $res_imagenes->fetch_assoc()) {
        $sql_imagenes = $row_sentencia['campos'] . $row_sentencia['tablas'] . $row_sentencia['condiciones'];
    }
    $res_imagenes = $mysqli1->query($sql_imagenes);
    while ($row_circulares = $res_imagenes->fetch_assoc()) {
        $html .=    '<div class="row align-items-center justify-content-center mb-5">';
    ...
    $html .= '<section class="container mb-5">';
